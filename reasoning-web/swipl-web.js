const SWIPL = (() => {
  ...
  return (
    function(SWIPL) {
      ...
      const Module = typeof SWIPL != 'undefined' ? SWIPL : {};
      ...
      (function() {
        ...
        const loadPackage = function(metadata) {
          ...
          const PACKAGE_NAME = 'src/swipl-web.data';
          const REMOTE_PACKAGE_BASE = 'swipl-web.data';
      ...
      const noExitRuntime = Module['noExitRuntime'] || false;
      ...
      function cwrap(ident, returnType, argTypes, opts) {
        argTypes = argTypes || [];
        const numericArgs = argTypes.every((type) => type === 'number' || type === 'boolean');
      ...
      Prolog.prototype._initialise = function() {
        ...
        const argv0 = this.args || [];
        argv0.unshift('swipl');
        const argv = argv0.map(function(arg) {
      ...
      function toJSON(prolog, term, options) {
        switch (prolog.bindings.PL_term_type(term)) {
          ...
          case prolog.PL_RATIONAL: {
            const s = prolog.get_chars(term, this.CVT_RATIONAL);
            const a = s.split('r');
            return {
              '$t': 'r',
              'v': {
          ...
          case prolog.PL_DICT: {
            ...
            for (var i = 2; ; i += 2) {
              if (prolog.get_arg(i + 1, term, a)) {
                const key = toJSON(prolog, a, options);
                prolog.get_arg(i, term, a);
